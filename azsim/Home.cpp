      ...
      TObject *Sender)
{
    try{
        AnsiString SQL_FILTRO =
          "SELECT DISTINCT LL.CDCLIENTE, C.NMCLIENTE, C.ENDERECO, C.BAIRRO, C.CIDADE "
          "FROM LOGULTIMOESTADO LL "
          "	JOIN CLIENTE C ON C.CDCLIENTE = LL.CDCLIENTE "
          "	JOIN CONTRATO CO ON CO.CDCLIENTE = LL.CDCLIENTE AND CO.INATIVO = 0 "
          "WHERE LL.CDCLIENTE NOT IN(SELECT l.CDCLIENTE FROM LOGULTIMOESTADO L "
          "	WHERE L.DATACADASTRO > DATEADD(-7 DAY TO CURRENT_DATE) "
          "		AND L.IDENTIFICACAO IS NOT NULL) "
          "	AND LL.DATACADASTRO > DATEADD(-7 DAY TO CURRENT_DATE) AND LL.IDENTIFICACAO IS NULL "
          "ORDER BY LL.CDCLIENTE";
